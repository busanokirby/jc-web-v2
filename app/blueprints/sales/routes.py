    ...
    if request.method == "POST":
        try:
            ...
            if not claim_on_credit:
                db.session.add(SalePayment(sale_id=sale.id, amount=total, method=payment_method, paid_at=datetime.now()))
    ...
    if q or status or date_from or date_to:
        ...
        try:
            sales_base = Sale.query.options(
                ...
                joinedload(Sale.payments),  # ✅ ADD: prevents N+1 and supports payment_methods in search_text
            )
            if status:
                sales_base = sales_base.filter(Sale.status.ilike(status.upper()))
            if date_from:
                sales_base = sales_base.filter(Sale.created_at >= date_from)  # ✅ FIX
            if date_to:
                sales_base = sales_base.filter(Sale.created_at <= date_to)  # ✅ FIX
            ...
            if pattern:
                sales_invoice = (
                    sales_base.filter(Sale.invoice_no.ilike(pattern))
                ...
                sales_payment = (
                    sales_base.join(SalePayment)
                    .filter(SalePayment.method.ilike(pattern))
                    .order_by(Sale.created_at.desc())
                    .distinct()
                    .limit(fetch_limit)
                    .all()
        ...
        try:
            ...
            if date_from:
                devices_base = devices_base.filter(
                    or_(
                        Device.actual_completion >= date_from.date(),
                        Device.created_at >= date_from
                    )
                )
            if date_to:
                devices_base = devices_base.filter(
                    or_(
                        Device.actual_completion <= date_to.date(),
                        Device.created_at <= date_to
                    )
                )
            ...
            if pattern:
                devices_ticket = (
                    devices_base.filter(Device.ticket_number.ilike(pattern))
                    .order_by(Device.created_at.desc())
                    .limit(fetch_limit)
                    .all()
                )
                devices_service = (
                    devices_base.filter(Device.service_type.ilike(pattern))
                    .order_by(Device.created_at.desc())
                    .limit(fetch_limit)
                    .all()
                )
                ...
                devices_product = (
                    devices_base.join(RepairPartUsed).join(Product)
                    .filter(or_(Product.name.ilike(pattern), Product.sku.ilike(pattern)))
                    .order_by(Device.created_at.desc())
                    .distinct()
                    .limit(fetch_limit)
        ...
        # Regular recent sales
        try:
            sales = Sale.query.options(
                joinedload(Sale.items).joinedload(SaleItem.product),
                joinedload(Sale.customer),
                joinedload(Sale.payments),  # ✅ ADD
            ).order_by(Sale.created_at.desc()).limit(200).all()
        except Exception as e:
            app.logger.error(f"Sales fetch error: {str(e)}")
            sales = []

        # Devices that have monetary transactions or released on credit
        try:
    ...
    try:
        for s in sales:
            try:
                ...
                parts_text = " ".join(
                    f"{item.product.sku or ''} {item.product.name or ''}"
                    for item in (s.items or [])
                    if item.product
                )
                customer_name = s.customer.display_name if getattr(s, "customer", None) else ""
                customer_business = s.customer.business_name if getattr(s, "customer", None) and getattr(s.customer, "business_name", None) else ""
                customer_id = s.customer.id if getattr(s, "customer", None) else None
                ...
                search_text = f"{s.invoice_no or ''} {customer_name} {customer_business} {customer_code} {customer_phone} {parts_text} {payment_methods}"

                payments_total = sum((p.amount or 0) for p in (s.payments or []))
                balance_due = float((s.total or 0) - payments_total)

                status_raw = (s.status or "").upper()
                claimed = getattr(s, "claimed_on_credit", False)
                if claimed and status_raw == "PARTIAL":
                    status_display = "PARTIAL/CREDIT"
                elif claimed:
    ...
    try:
        for d in devices:
            try:
                ...
                parts_text = " ".join(
                    f"{p.product.sku or ''} {p.product.name or ''}"
                    for p in (d.parts_used_rows or [])
                    if p.product
                )
                customer_name = d.owner.display_name if getattr(d, "owner", None) else ""
                customer_business = d.owner.business_name if getattr(d, "owner", None) and getattr(d.owner, "business_name", None) else ""
                customer_id = d.owner.id if getattr(d, "owner", None) else None

                # Normalize actual_completion (Date) to datetime for sorting
                if d.actual_completion:
                    if isinstance(d.actual_completion, datetime):
                        tx_date = d.actual_completion
                    else:
                        tx_date = datetime.combine(d.actual_completion, datetime.min.time())
                else:
                    tx_date = d.created_at

                search_text = f"{d.ticket_number or ''} {customer_name} {customer_business} {parts_text} {d.service_type or ''}"

                status_display = (
                    "PARTIAL/CREDIT"
                    if getattr(d, "claimed_on_credit", False) and (d.payment_status or "").upper() == "PARTIAL"
                    else (
                        "CREDIT"
                        if getattr(d, "claimed_on_credit", False)
                        else (d.payment_status or "").upper()
                    )
                )
...
@roles_required('ADMIN', 'SALES')
def credits():
    """List devices released on credit (outstanding claims) + partials."""
    # Include both claimed-on-credit items AND partial payments as 'credits'
    ...
    devices_on_credit = (
        ...
        .filter(Device.claimed_on_credit == True)
        .order_by(Device.created_at.desc())
        .all()
    )
    ...
    # Also include partial sales/repairs
    sales_partial = (
        Sale.query
        .options(
            joinedload(Sale.customer),
        ...
        .filter(Sale.status.ilike('PARTIAL'))
        .order_by(Sale.created_at.desc())
        .all()
    )
    ...
    devices_partial = (
        ...
        .filter(Device.payment_status.ilike('Partial'))
        .order_by(Device.created_at.desc())
        .all()
    )

    # Merge lists without duplicates (by id and type)
    history = []
    seen = set()

    def add_repair_entry(d, claimed_flag=False):
        key = ('repair', d.id)
        if key in seen:
            return
        seen.add(key)

        parts_qty = sum(p.qty for p in (d.parts_used_rows or []))
        parts_text = ' '.join(
            f"{p.product.sku or ''} {p.product.name or ''}"
            for p in (d.parts_used_rows or [])
            if p.product
        ...
        balance = float(d.balance_due or 0)

        # compute display status: merge partial + credit
        raw = (d.payment_status or '').upper()
        if claimed_flag and raw == 'PARTIAL':
            sdisp = 'PARTIAL/CREDIT'
        elif claimed_flag:
            sdisp = 'CREDIT'
        else:
            sdisp = raw

        # ✅ FIX: include customer fields so template does NOT show Walk-in
        customer_name = d.owner.display_name if getattr(d, 'owner', None) else ''
        customer_business = (
            d.owner.business_name
            if getattr(d, 'owner', None) and getattr(d.owner, 'business_name', None)
            else ''
        )
    ...
    def add_sale_entry(s, claimed_flag=False):
        key = ('sale', s.id)
        if key in seen:
            return
        seen.add(key)

        items_count = sum(item.qty for item in (s.items or []))
        parts_text = ' '.join(
            f"{item.product.sku or ''} {item.product.name or ''}"
            for item in (s.items or [])
            if item.product
        ...
        customer_business = (
            ...
            if getattr(s, 'customer', None) and getattr(s.customer, 'business_name', None)
            else ''
        )
    ...
    # Add claimed-on-credit repairs first
    for d in devices_on_credit:
        add_repair_entry(d, claimed_flag=True)

    # Add claimed-on-credit sales
    for s in sales_on_credit:
        add_sale_entry(s, claimed_flag=True)

    # Add partial repairs (if not already added)
    for d in devices_partial:
        add_repair_entry(d, claimed_flag=getattr(d, 'claimed_on_credit', False))

    # Add partial sales
    for s in sales_partial:
        add_sale_entry(s, claimed_flag=getattr(s, 'claimed_on_credit', False))

    # Compute totals and counts
    credits_count = len(history)
    credits_total = sum(item.get('balance_due', 0) for item in history)
...
def add_payment_for_sale(sale_id: int):
    ...
    accepted = amount if amount <= remaining else remaining  # ✅ FIX
    db.session.add(SalePayment(sale_id=sale.id, amount=accepted, method=payment_method, paid_at=datetime.now()))

    new_paid_total = payments_total + accepted
    if new_paid_total >= (sale.total or 0):  # ✅ FIX
        sale.status = "PAID"
    ...
    if accepted < amount:  # ✅ FIX
        flash(
            f"Payment exceeded remaining amount. Accepted ₱{accepted} and capped to total. "
            f"Excess ₱{(amount - accepted)} not recorded.",
            "warning"
        )
    else:
...
def daily_sales():
    ...
    for pay in sale_payments:
        ...
        if amount <= 0:
            continue  # template is "received payments only" [1](https://depedph-my.sharepoint.com/personal/carmen_busano01_deped_gov_ph/Documents/Old%20Files/Microsoft%20Copilot%20Chat%20Files/daily_sales.html)
...
def reports():
    ...
    sales_period = (
        ...
        .filter(Sale.created_at >= start_date)  # ✅ FIX
        .all()
    )
    ...
    repairs_period_devices = Device.query.filter(
        ...
        Device.actual_completion >= start_date_only  # ✅ FIX
    ).all()

    repair_parts_period = (
        db.session.query(RepairPartUsed).join(Device, RepairPartUsed.device_id == Device.id)
        .filter(Device.actual_completion != None, Device.actual_completion >= start_date_only)  # ✅ FIX
        .all()
    )
    ...
    avg_transaction = (total_revenue / total_transactions) if total_transactions > 0 else Decimal("0.00")  # ✅ FIX

    payment_methods = db.session.query(
        SalePayment.method,
        func.count(SalePayment.id).label("count"),
        func.sum(SalePayment.amount).label("total")
    ).join(Sale).filter(Sale.created_at >= start_date).group_by(SalePayment.method).all()  # ✅ FIX
    ...
    daily_trend = [
        {"date": str(dt), "count": data["count"], "total": data["total"]}
        for dt, data in sorted(daily_totals.items())
    ]
